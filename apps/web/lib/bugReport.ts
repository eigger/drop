...
// 모아뒀다가 제보 시 자동으로 붙여준다. 이 저장소는 공개(public)라서 파일명/이메일 같은
// 개인정보가 될 만한 값은 절대 자동 수집하지 않고, 경로·상태코드·에러 메시지 같은 기술
...
const BUG_REPORT_REPO = "eigger/drop";
...
export function buildBugReportUrl(params: { title: string; description: string; pathname: string }): string {
  ...
  lines.push("");
  lines.push("---");
  lines.push(`**앱 버전:** v${process.env.NEXT_PUBLIC_APP_VERSION ?? "unknown"}`);
