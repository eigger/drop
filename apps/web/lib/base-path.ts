// 앱이 항상 오리진 루트에 있는 것은 아니다. 리버스 프록시의 서브패스(https://example.com/drop/)나
...
/** 루트 기준 절대경로에 배포 프리픽스를 붙인다. `/sw.js` → `/drop/sw.js` */
 ...
 * `startsWith(basePath)`만 쓰면 `/drop`이 `/dropdown`까지 먹는다.
 */
